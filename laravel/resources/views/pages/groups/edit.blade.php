...
@section('content')
    <div class="max-w-screen-md mx-auto p-4">
        ...
        <div class="flex items-center justify-between mb-3">
            <h2 class="font-semibold text-xl mb-3">Group id:{{$group->id}}</h2>
            <a href="{{ url()->previous() }}"
        ...
        <form action="{{ route('groups.update', $group) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-6">
                ...
                <input type="text" id="name" name="name"
                       ...
                       @enderror
                       placeholder="User name" value="{{ old('name', $group->name) }}">
        ...
        @can('delete', $group)
            <form method="post"
                  id="remove-from"
                  action="{{route('groups.destroy', $group)}}" style="display: none">
                @csrf
                @method('DELETE')
            </form>
        @endif
    ...
    <script>
        ...
        if (btn && form) {
            btn.addEventListener('click', function (event) {
                ...
                if (confirm('Are you sure you want to delete the this group?'))
                    form.submit();
            })
        }
    </script>
