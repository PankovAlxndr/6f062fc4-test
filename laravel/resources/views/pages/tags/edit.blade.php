...
@section('content')
    <div class="max-w-screen-md mx-auto p-4">
        ...
        <div class="flex items-center justify-between mb-3">
            <h2 class="font-semibold text-xl mb-3">Tag id:{{$tag->id}}</h2>
            <a href="{{ url()->previous() }}"
        ...
        <form action="{{ route('tags.update', $tag) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-6">
                <label for="name"
                       class="block mb-2 text-sm font-medium @error('name') text-red-700 dark:text-red-500 @else text-gray-900 dark:text-white @enderror">Tag
                    name</label>
                <input type="text" id="name" name="name"
                       ...
                       @enderror
                       placeholder="User name" value="{{ old('name', $tag->name) }}">
        ...
        <form method="post"
              id="remove-from"
              action="{{route('tags.destroy', $tag)}}" style="display: none">
            @csrf
            @method('DELETE')
        </form>
    </div>
    ...
    <script>
        ...
        if (btn && form) {
            btn.addEventListener('click', function (event) {
                ...
                if (confirm('Are you sure you want to delete the this tag?'))
                    form.submit();
            })
        }
    </script>
